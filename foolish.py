...
def simulated_annealing_no_worse(values, weights, capacity, initial_temp, alpha, beta, i0, max_iterations):
    # Start with an empty knapsack (valid but value 0)
    S = [0 for _ in values]
    T = initial_temp
    iterations = i0
    best_solution = S[:]
    ...
    while total_outer < max_iterations:
        ...
        for _ in range(iterations):
            ...
            # Only accept better solutions (strict hill climbing)
            if h_NewS < h_S:
                S = NewS
...
if __name__ == "__main__":
    ...
    best_sol, best_val = simulated_annealing_no_worse(
        values, weights, capacity,
        initial_temp=10, alpha=0.98, beta=1.02, i0=1000, max_iterations=100
    )
